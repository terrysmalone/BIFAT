...
{
    public partial class LayerGroupsForm : Form
    {
        private IModel _model;
        private List<Tuple<string, Color>> layerGroups;

        private Tuple<string, Color> selectedGroup;

        public LayerGroupsForm(IModel _model)
        {
            this._model = _model;
            InitializeComponent();

            layerGroups = _model.GetLayerGroups();
        ...
        private void DisplayGroups()
        {
            ...
            {
                ListViewItem row = new ListViewItem(layerGroups[i].Item1);

                row.SubItems.Add(_model.GetLayerGroupCount(layerGroups[i].Item1).ToString());
                groupsListView.Items.Add(row);
                groupsListView.Items[i].BackColor = layerGroups[i].Item2;
            }
        }
        ...
        private void groupsListView_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (groupsListView.SelectedItems.Count > 0)
            {
                ...
                selectedGroup = layerGroups[groupsListView.SelectedIndices[0]];
            }
            else
            {
        ...
        private void changeColourButton_Click(object sender, EventArgs e)
        {
            ...
            if (colourPicker.ShowDialog() == DialogResult.OK)
            {
                ...
                _model.ChangeLayerGroupColour(selectedGroup.Item1, newColour);
        ...
        private void addGroupButton_Click(object sender, EventArgs e)
        {
            ...
            _model.AddLayerGroup(nameForm.GetName());
            _model.SaveGroups();

            DisplayGroups();

            selectedGroup = layerGroups[groupsListView.Items.Count - 1];
        ...
        private void renameGroupButton_Click(object sender, EventArgs e)
        {
            ...
            _model.RenameLayerGroup(selectedGroup.Item1, nameForm.GetName());

            DisplayGroups();

            selectedGroup = layerGroups[selectedIndex];
        ...
        {
            _model.DeleteLayerGroup(selectedGroup.Item1);
