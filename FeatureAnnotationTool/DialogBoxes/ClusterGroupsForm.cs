...
{
    public partial class ClusterGroupsForm : Form
    {
        private IModel _model;
        private List<Tuple<string, Color>> clusterGroups;

        private Tuple<string, Color> selectedGroup;

        public ClusterGroupsForm(IModel _model)
        {
            this._model = _model;
            InitializeComponent();

            clusterGroups = _model.GetClusterGroups();
        ...
        private void DisplayGroups()
        {
            ...
            {
                ListViewItem row = new ListViewItem(clusterGroups[i].Item1);

                row.SubItems.Add(_model.GetClusterGroupCount(clusterGroups[i].Item1).ToString());
                groupsListView.Items.Add(row);
                groupsListView.Items[i].BackColor = clusterGroups[i].Item2;
            }
        }
        ...
        private void groupsListView_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (groupsListView.SelectedItems.Count > 0)
            {
                ...
                selectedGroup = clusterGroups[groupsListView.SelectedIndices[0]];
            }
            else
            {
        ...
        private void changeColourButton_Click(object sender, EventArgs e)
        {
            ...
            if (colourPicker.ShowDialog() == DialogResult.OK)
            {
                ...
                _model.ChangeClusterGroupColour(selectedGroup.Item1, newColour);
        ...
        private void addGroupButton_Click(object sender, EventArgs e)
        {
            ...
            _model.AddClusterGroup(nameForm.GetName());
            _model.SaveGroups();

            DisplayGroups();

            selectedGroup = clusterGroups[groupsListView.Items.Count - 1];
        ...
        private void renameGroupButton_Click(object sender, EventArgs e)
        {
            ...
            _model.RenameClusterGroup(selectedGroup.Item1, nameForm.GetName());

            DisplayGroups();

            selectedGroup = clusterGroups[selectedIndex];
        ...
        {
            _model.DeleteClusterGroup(selectedGroup.Item1);
